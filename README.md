...
- export `MPI_PKG_CONFIG` to be the name or path for pkg-config for your implementation
  ...
  - The 2 easiest options for running rsmpi are through `cargo mpirun` or directly through `cargo run` (or the equivalent binary generated by `cargo build`)
    - To use `cargo mpirun`, install it with `cargo install cargo-mpirun`, and then load mpi modules with `module load mpi/openmpi-x86_64`
    - To run the binary directly, you need to set 2 environmental variables to paths that may vary between distros. On Fedora 41, this involves setting `$PKG_CONFIG_PATH` to `/usr/lib64/mpich/lib/pkgconfig` and `$LD_LIBRARY_PATH` to `/usr/lib64/mpich/lib/` after installing `mpich` through `dnf`
